    ...
    public static void main(String[] args) throws Exception{
        ...
        try (Stream<String> lines =  Files.lines(Paths.get(stateMachineSource))){
            lines.forEach(line->{ //A   B   STATE   TRANSITION_RULE
                ...
                calculated.add("'st"+parts[0]+"_"+parts[1]+"', ("+parts[3]+")::text ");
        ...
        final String query = "select *, fsm(hstore(ARRAY[" + String.join(", ", calculated) + "])) over (PARTITION BY name ORDER BY age) from life order by name,age"; //over (PARTITION BY name ORDER BY age)
